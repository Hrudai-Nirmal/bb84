...
online_users = {}
shared_keys = {}
eavesdroppers = {}  # Maps (user1, user2) -> set of eavesdropper_sids
lock = Lock()
...
def get_shared_key(user1, user2):
    ...
    with lock:
        active_eavesdroppers = eavesdroppers.get(pair, set())
        
    if active_eavesdroppers:
        # Eavesdropper detected!
        print(f"⚠️ Eavesdropper detected on channel {pair}!")
        return None, True  # key, is_compromised
    ...
    with lock:
        if pair not in shared_keys:
            ...
            # Notify potential eavesdroppers that a new channel is active
            socketio.emit("channel_active", {"user1": pair[0], "user2": pair[1]})
            
        return shared_keys[pair], False
...
def login():
    ...
    is_eavesdropper = data.get("is_eavesdropper", False)
    
    print(f"Login attempt for user: {username}, eavesdropper: {is_eavesdropper}")
    ...
    with app.app_context():
        ...
        if user:
            ...
            if check_password_hash(user.password_hash, password):
                ...
                session['is_eavesdropper'] = is_eavesdropper
                session.permanent = True
                return jsonify({"success": True, "username": username, "is_eavesdropper": is_eavesdropper})
            else:
...
def check_session():
    ...
    is_eavesdropper = session.get('is_eavesdropper', False)
    if username:
        return jsonify({"logged_in": True, "username": username, "is_eavesdropper": is_eavesdropper})
...
def handle_disconnect():
    ...
    # Also remove from eavesdroppers list
    with lock:
        for pair in eavesdroppers:
            if request.sid in eavesdroppers[pair]:
                eavesdroppers[pair].remove(request.sid)
                
    print(f"Client disconnected: {request.sid}")

@socketio.on("start_eavesdropping")
def handle_start_eavesdropping(data):
    ...
    pair = tuple(sorted((target1, target2)))
    
    with lock:
        if pair not in eavesdroppers:
            eavesdroppers[pair] = set()
        eavesdroppers[pair].add(request.sid)
        
    print(f"Spy {request.sid} started eavesdropping on {pair}")
    emit("eavesdropping_started", {"target1": target1, "target2": target2})

@socketio.on("stop_eavesdropping")
def handle_stop_eavesdropping(data):
    target1 = data.get("target1")
    target2 = data.get("target2")
    ...
    with lock:
        if pair in eavesdroppers and request.sid in eavesdroppers[pair]:
            eavesdroppers[pair].remove(request.sid)
            
    print(f"Spy {request.sid} stopped eavesdropping on {pair}")
    emit("eavesdropping_stopped", {"target1": target1, "target2": target2})

@socketio.on("get_active_channels")
def handle_get_active_channels():
    # Return list of pairs that have shared keys or just online users pairs
    # For simplicity, let's return pairs that have generated keys
    active_pairs = []
    with lock:
        for pair in shared_keys.keys():
            # Only show if both users are online (optional, but makes sense)
            if pair[0] in online_users and pair[1] in online_users:
                active_pairs.append(pair)
    
    emit("active_channels", active_pairs)
...
def handle_send_message(data):
    ...
    key, compromised = get_shared_key(sender, recipient)
    
    if compromised:
        # Notify users of eavesdropping
        alert_msg = {
            "type": "security_alert",
            "message": "⚠️ QUANTUM INTERCEPTION DETECTED! Communication compromised."
        }
        if sender in online_users:
            emit("security_alert", alert_msg, room=online_users[sender])
        if recipient in online_users:
            emit("security_alert", alert_msg, room=online_users[recipient])
            
        # Notify eavesdroppers (they get garbage/alert)
        pair = tuple(sorted((sender, recipient)))
        with lock:
            spies = eavesdroppers.get(pair, set())
            for spy_sid in spies:
                emit("intercept_result", {
                    "success": False,
                    "message": "Measurement disturbed the quantum state. You were detected!",
                    "sender": sender,
                    "recipient": recipient
                }, room=spy_sid)
        return
...
def handle_get_history(data):
    ...
    key, compromised = get_shared_key(user1, user2)
    
    if compromised:
        emit("chat_history", {"history": [], "key": None, "error": "Channel compromised by eavesdropper"})
        return
